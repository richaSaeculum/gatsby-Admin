...
const StatusCard: React.FC<Props> = ({
    ...
    return (
        <div className={`${layout}`}>
            <div className={`card bg-${bgColor} ${className} statuscard `}>
                <div className='card-body d-flex align-items-center gap-9'>
                    {/* <KTSVG path={svgIcon} className={`svg-icon-${iconColor} svg-icon-3x ms-n1`} /> */}
                    <i className={`fa-solid fa-3x ${faIcon}`} ></i>
                    <div>
                        <div className={`text-${titleColor} fw-semibold mb-2`}>{title}</div>
